...
\end{equation}
and the code uses a \emph{doubled} real-space unit cell:
\begin{equation}
  A_{\mathrm{uc}} = 2\, A_{\mathrm{prim}} = \sqrt{3}\, L_{\mathrm{moire}}^2.
\end{equation}
The magnetic flux per doubled unit cell is
$\Phi/\Phi_0 = qq/pp$, and the magnetic field is
$B = (qq/pp)\,\Phi_0 / A_{\mathrm{uc}}$.

The magnetic Brillouin zone (MBZ) is $1/pp$ the area of the moire BZ.
Due to the doubled real-space cell, the moire BZ is itself half the
``primary'' moire BZ.  Therefore the MBZ covers a fraction $1/(2\,pp)$
of the primary moire BZ.

Eigenvalues are computed on a uniform $\mathbf{k}$-mesh of $\Nk$
points covering the MBZ.  The \emph{physical} DOS per primary unit
cell requires dividing the eigenvalue sum by the number of primary
unit cells represented:
\begin{equation}\label{eq:Nbz}
  N_\BZ = \Nk \times 2\,pp.
\end{equation}
The factor $2\,pp$ accounts for $pp$-fold BZ folding and the factor of
2 from the doubled cell.
...
\begin{equation}
  \sigma_{xx} = \frac{4\,pp\,\hbar^2}{A_m\,\Nk}
  \times \sum_\mathbf{k} \Phi_{xx}(\mu) \quad
  [\text{units of } e^2/h],
\end{equation}
where $A_m = pp^2\, A_{\mathrm{uc}} / (2\,qq)$ is the magnetic unit
cell area in~$\Ang^2$, and the factor of 4 absorbs the spin
degeneracy times the $\pi$ factors from the spectral functions
($4 = 2_{\mathrm{spin}} \times 2/\pi \times \pi$; see derivation
below).  In SCBA mode the $\Gam^2$ is already inside $\Phi_{xx}$, so
\texttt{pf\_xx} omits it (line~812):
\begin{verbatim}
  pf_xx = 4.0 * pp * HBAR_EV**2 / (A_m_Ang2 * Nk_tot)   # SCBA
  pf_xx = 4.0 * pp * HBAR_EV**2 * G2 / (A_m_Ang2 * Nk_tot)  # constant
\end{verbatim}
...
\end{equation}
Including spin degeneracy ($\times 2$) and the BZ average
($\Omega \to A_m / pp$, with $1/\Nk$ sum over BZ):
\begin{equation}
  \frac{\sigma_{xx}}{e^2/h}
  = \frac{4\, pp\, \hbar^2}{A_m\,\Nk}\, \Gam^2\,
